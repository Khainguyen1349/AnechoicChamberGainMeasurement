...
for ant = 1:4
    for i = 1:7
        freq = 240 + i*5;
        filename = strcat('Ant',num2str(ant),'_FF_dir_0',num2str(freq),'00.TXT');
        f = fopen(filename,'r');
        nLine = 0;
        k = 0;
        h = 0;
...
Directivity.AmplitudeCo = AmplitudeCo;
Directivity.PhaseCo = PhaseCo;
Directivity.AmplitudeCross = AmplitudeCross;
Directivity.PhaseCross = PhaseCross;

save('AntennaDirectivity.mat', 'Directivity')
